...
import pytest
import importlib
from agents.agent_random.random_agent import generate_move_random, MCTSNode, EXPLORATION_COEF, simulate, RANDOM
from game_utils import BOARD_ROWS, BOARD_COLS, PLAYER1, PLAYER2, apply_player_action, check_end_state, GameState, \
    SavedState, PlayerAction
...
def test_update_multiple_nodes_independent():
    ...
    node2.update(result_player=PLAYER1)
...
def test_simulate_detects_draw_on_full_board():
    # Create a full board with no winners (draw)
    board = create_empty_board()
    # Fill board alternating players without making 4 in a row
    for c in range(BOARD_COLS):
        for r in range(BOARD_ROWS):
            # Alternate piece by column+r to avoid 4 in a row in any direction
            player = PLAYER1 if (c + r) % 2 == 0 else PLAYER2
...
def test_simulate_random_playout(monkeypatch):
    # Test that simulate alternates players and eventually ends
    board = create_empty_board()
    # Force a simple random behavior: always choose the first available column
    monkeypatch.setattr(RANDOM, 'choice', lambda valid: valid[0])
    # Run a playout starting with PLAYER1
    result = simulate(board, PLAYER1)
    # The result should be either PLAYER1, PLAYER2, or None
    assert result in (PLAYER1, PLAYER2, None)
    # Ensure simulate terminates

# TODO: skh, see what's wrong with this test

def test_fallback_when_no_iterations(monkeypatch):
    board = create_empty_board()
    # Force zero iterations to skip MCTS and go to fallback
    monkeypatch.setattr('random_agent.ITERATIONS', 0)
    # Monkeypatch RANDOM.choice to pick column 3
    monkeypatch.setattr(RANDOM, 'choice', lambda valid: 3)
...
def test_selects_expanded_move(monkeypatch):
    ...
    monkeypatch.setattr('random_agent.ITERATIONS', 1)
    # Make simulate fast and deterministic: always return current player (no effect backprop other than wins)
    monkeypatch.setattr('random_agent.simulate', lambda state, player: player)
    # Run agent
    action, state = generate_move_random(board, PLAYER1, None)
    # After one expansion, the popped column is BOARD_COLS-1
    expected_col = BOARD_COLS - 1
    assert action.column == expected_col, f"Expected expanded move {expected_col}, got {action.column}"
    assert state is None
...
def test_saved_state_preserved(monkeypatch):
    ...
    monkeypatch.setattr('random_agent.ITERATIONS', 0)
    monkeypatch.setattr(RANDOM, 'choice', lambda valid: valid[0])
    dummy_state = SavedState(move_count=42)
